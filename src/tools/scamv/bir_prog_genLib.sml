  ...
  fun gen_until_liftable retry_on_liftfail arch_str prog_gen_fun args =
    ...
    let
      ...
      *)
      ...
      fun lbl_addr idx =
        case arch_str of
            "m0" => ``BL_Address (Imm32 ^(mk_wordi (Arbnum.fromInt (idx*2), 32)))``
          | _ => ``BL_Address (Imm64 ^(mk_wordi (Arbnum.fromInt (idx*4), 64)))``;
      fun lbl_exists idx = List.exists (fn x => x = lbl_addr idx) labels;
    ...
    let
      ...
      val prog_with_halt =
        let
          ...
          val lbl =
            case arch_str of
                "m0" => ``BL_Address (Imm32 ^(mk_wordi (Arbnum.fromInt (len*2), 32)))``
              | _ => ``BL_Address (Imm64 ^(mk_wordi (Arbnum.fromInt (len*4), 64)))``;
...
fun prog_gen_store_rand param arch_str sz =
  let val prog_gen_fun =
    case arch_str of
        "m0" => (fn _ => fn _ => [(*"cmp r2, r3", "beq end",*)
        (*"mov r4, #42",*) "ldr r4, [r5]"(*, "end:", "add r4, #5"*)])
      | _ => (fn _ => fn _ => [(*"cmp x2, x3", "beq end", "mov x4, #42",*)
      "ldr x4, [x5]"(*, "end:", "add x4, x4, #5"*)])
     (* | _   => bir_prog_gen_arm8_rand *)
  in prog_gen_store ("prog_gen_rand::"^param) false arch_str (prog_gen_fun param) sz
  (* TODO *)
  (* in prog_gen_store ("prog_gen_rand::"^param) true arch_str (prog_gen_fun
  * param) sz*)
  end;
